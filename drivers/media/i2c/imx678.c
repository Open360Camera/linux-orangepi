...
static const struct regval imx678_tpg_enable_regs[] = {
        //TPG config
        { 0x30DC, 0x00 }, //BLKLVL_LOW
        { 0x3042, 0x00 }, //XSIZE_OVERLAP
        { 0x30e0, 0x01 }, //TPG_EN_DUOUT
        { 0x30e4, 0x00 }, //TPG_COLORWIDTH
        { 0x5300, 0x0A }, //TST_CLK_EN
...
static const struct regval imx678_tpg_disable_regs[] = {
        //TPG config
        { 0x30DC, 0x32 }, //BLKLVL_LOW
        { 0x3042, 0x00 }, //XSIZE_OVERLAP
        { 0x30e0, 0x00 }, //TPG_EN_DUOUT
        { 0x30e4, 0x00 }, //TPG_COLORWIDTH
        { 0x5300, 0x02 }, //TST_CLK_EN
                                  ...
                                  struct rkmodule_inf *inf)
{
        ...
        strlcpy(inf->base.module, "IMX678", sizeof(inf->base.module));
        strlcpy(inf->base.lens, "Leica", sizeof(inf->base.lens));

        dev_dbg(&imx678->client->dev,
...
static long imx678_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
        ...
        dev_err(&imx678->client->dev, "ioctl call %d\n", cmd);
        dev_err(&imx678->client->dev, "ioctl RKMODULE_GET_MODULE_INFO %ld\n", RKMODULE_GET_MODULE_INFO);
        dev_err(&imx678->client->dev, "ioctl PREISP_CMD_SET_HDRAE_EXP %ld\n", PREISP_CMD_SET_HDRAE_EXP);
        ...
        printk(KERN_INFO "imx678: imx678_ioctl => %ld\n", ret);
                                  ...
                                  unsigned int cmd, unsigned long arg)
{
        ...
        printk(KERN_INFO "imx678: imx477_ioctl => %ld\n", ret);
...
static int __imx678_power_on(struct imx678 *imx678)
{
        ...
        #define IMX678_XCLR_MIN_DELAY_US        10000
                                ...
                                struct v4l2_subdev_selection *sel)
{
        ...
        if (sel->target == V4L2_SEL_TGT_CROP_BOUNDS) {
                ...
                dev_dbg(&imx678->client->dev, "imx678_get_selection: %d, %d, %d, %d\n",
                        sel->r.left, sel->r.width, sel->r.top, sel->r.height);
...
{
        int ret = 0;
        dev_dbg(&imx678->client->dev, "imx678_set_test_pattern: %d\n", val);

        if (TEST_PATTERN_DISABLED == val)
        {
                ret = imx678_write_array(imx678->client, imx678_tpg_enable_regs);
                if (!ret) {
                        ret = imx678_write_reg(imx678->client, IMX678_TPG_PATSEL_DUOUT, 1, val - 1);
                }
        }
        else {
                ret = imx678_write_array(imx678->client, imx678_tpg_disable_regs);
        }
        return ret;
}
...
static int imx678_set_ctrl(struct v4l2_ctrl *ctrl)
{
        ...
        case V4L2_CID_TEST_PATTERN:
                // if (!pm_runtime_get_if_in_use(&imx678->client->dev)) {
                //      dev_err(&imx678->client->dev, "imx678_set_ctrl pm_runtime_get_if_in_use failed with code %d\n", ret);
                //      return 0;
                // }
                ret = imx678_set_test_pattern(imx678, ctrl->val);

                // pm_runtime_put(&imx678->client->dev);
...
static int imx678_initialize_controls(struct imx678 *imx678)
{
        ...
        imx678->test_pattern = v4l2_ctrl_new_std_menu_items(handler,
                                        &imx678_ctrl_ops, V4L2_CID_TEST_PATTERN,
                        ...
                        const struct i2c_device_id *id)
{
        ...
        imx678->pwdn_gpio = devm_gpiod_get(dev, "pwdn", GPIOD_OUT_HIGH);
        if (IS_ERR(imx678->pwdn_gpio))
